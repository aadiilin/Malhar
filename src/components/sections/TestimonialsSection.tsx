  ...
  return (
    <section className="section-padding bg-emerald-900 text-white relative overflow-hidden">
      ...
      <div className="relative z-10 container-custom">
        <AnimatedSection>
          <span className="inline-block px-4 py-1.5 bg-white/10 text-amber-300 text-sm font-semibold rounded-full mb-4">Testimonials</span>
        ...
        <div className="max-w-3xl mx-auto">
          <AnimatedSection key={current}>
            <div className="text-center">
              ...
              <p className="text-xl md:text-2xl leading-relaxed text-emerald-100 mb-8 italic">
              ...
              <p className="text-emerald-300/70 text-sm">{testimonials[current].role}</p>
            </div>
          </AnimatedSection>
            ...
            <div className="flex gap-2">
              {testimonials.map((_, i) => (
                <button key={i} onClick={() => setCurrent(i)}
                  className={`w-2.5 h-2.5 rounded-full transition-all ${i === current ? "bg-amber-400 w-8" : "bg-white/30"}`} />
              ))}
            </div>
